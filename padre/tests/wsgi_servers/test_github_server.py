...
from padre.wsgi_servers import github

from webob import exc


class GithubHookApplicationTest(TestCase):
    def setUp(self):
        super(GithubHookApplicationTest, self).setUp()
        self.bot = common.make_bot()
        self.hook = github.HookApplication(self.bot)

    def test_creation(self):
        self.assertEqual(
            [(re.compile('^github-webhook[/]?(.*)$'),
    ...
    @mock.patch('padre.wsgi_servers.github.Request')
    def test_magic_call(self, request):
        env = dict()
        env['PATH_INFO'] = '.'
        resp = mock.MagicMock()
        resp.path.lstrip.return_value = 'github-webhook'
        resp.method = 'GET'
    ...
    @mock.patch('padre.wsgi_servers.github.Response')
    def test_reply_ping(self, response):
        resp = self.hook.reply_pong('test request')
        self.assertEqual('{\n    "msg": "pong"\n}\n', resp.body)
    ...
    @mock.patch('padre.wsgi_servers.github.Response')
    def test_hook_return_with_ping_kind(self, response):
        req = mock.MagicMock()
        req.headers.get.side_effect = ['ping', '10']
        self.hook.reply_pong('test request')
        self.assertEqual('{\n    "msg": "pong"\n}\n', self.hook.hook(req).body)
    ...
    def test_hook_successful_with_proper_data(self):
        ...
        req.content_type = 'application/json'
        req.content_length = 100
        req.body_file.read.return_value = b'{ "first": "data" }'
    ...
    def test_hook_raises_with_wrong_credentials(self):
        ...
        req.content_type = 'application/json'
        req.content_length = 100
        self.bot.config.github.hook.secret = 'secret'
        with ExpectedException(
